<?php
    /*
     * MOD History
     * v0.4 "img" is now a self-closinjg tag
     * v0.3.1 Changed the value used on implicit HTML attrivutes
     * v0.3 Added support for implicit HTML attributes
     * v0.2 Added @param to comment tag function to allow skip break line prefix
     * v0.1 Added DOCTYPE special tag for HTML5
     */
    ...
    class HtmlGen {
        /*
         * @var array
         * MOD: Added element "img"
         */
        static public $self_closing_tags = array(
            "base", "basefont", "br", "col", "frame", "hr", "input", "link", "meta", "param", "img"
        );
        ...
        static private function attributes(Array $attributes){
            ...
            return " ".implode(" ", Utility::array_kmap(function($k, $v){
                # MOD: implicit attributes are defined with a empty string as value
                if ($v !== '') {
         ...
         * MOD: @param boolean $break_line
         */
        static public function comment($comment, $break_line = TRUE){
            echo $break_line ? "\n" : '', self::indent(), "<!-- {$comment} -->\n";
            self::outdent();
        }
